...
function doGet(e) {
  ...
  try {
    ...
    switch(action) {
      ...
      default: result = { code: '000', error: '', data: { status: 'ok', message: 'API funguje', version: 'v2026-04-09' } };
    }
...
// v2026-04-09: přidán source_sheet jako r[18]
// 'záznamy' = nové záznamy, 'záznamy_historie' = historické
// updateRecord podle tohoto příznaku ví kam zapsat opravu
  ...
  try {
    ...
    if (source === 'new' || source === 'all') {
      ...
      for (let i = 1; i < records.length; i++) {
        ...
        let r = records[i].slice(0, 17);
        r.push(i);                        // r[17] = row_index
        r.push(SHEET_NAMES.RECORDS);      // r[18] = source_sheet
        data.push(r);
      }
    }
    if (source === 'history' || source === 'all') {
      ...
      for (let i = 1; i < records.length; i++) {
        ...
        let r = records[i].slice(0, 17);
        r.push(i);                               // r[17] = row_index
        r.push(SHEET_NAMES.RECORDS_HISTORY);     // r[18] = source_sheet
        data.push(r);
      }
    }
  ...
  try {
    ...
    for (let i = 1; i < records.length; i++) {
      ...
      if (recordDate >= targetDate && recordDate < nextDay) {
        ...
        const jeOpraveno = (stavP === 'opraveno' || stavP === 'nový' || stavP === 'rozpracováno') ? stavP : '';
        filtered.push([
...
// v2026-04-09: ROZCESTNÍK - zapisuje do správného listu podle source_sheet
// source_sheet = 'záznamy' → nové záznamy
// source_sheet = 'záznamy_historie' → historické záznamy
